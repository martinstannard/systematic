defmodule DashboardPhoenix.BranchMonitorTest do
      ...
      |> Enum.each(fn block ->
        case Regex.run(~r/worktree (.+)/, block) do
          [_, path] when path != test_dir ->
            System.cmd("git", ["worktree", "remove", path, "--force"],
              cd: test_dir,
              stderr_to_stdout: true
            )
  ...
  describe "git branch detection logic" do
    ...
    test "git branch --no-merged shows branches with unmerged commits", %{test_dir: test_dir} do
      ...
      {_, 0} =
        System.cmd("git", ["commit", "-m", "Feature commit"],
          cd: test_dir,
          stderr_to_stdout: true
        )
      ...
      {output, 0} =
        System.cmd("git", ["branch", "--no-merged", "main", "--format=%(refname:short)"],
          cd: test_dir
        )
    ...
    test "git branch --no-merged shows empty when all branches are merged", %{test_dir: test_dir} do
      ...
      {_, 0} =
        System.cmd("git", ["commit", "-m", "Will be merged"],
          cd: test_dir,
          stderr_to_stdout: true
        )
      ...
      {_, 0} =
        System.cmd("git", ["merge", "feature/merged", "--no-edit"],
          cd: test_dir,
          stderr_to_stdout: true
        )
    ...
    test "worktrees are detected even when branch is merged", %{test_dir: test_dir} do
      ...
      {_, 0} =
        System.cmd("git", ["commit", "-m", "Worktree commit"],
          cd: test_dir,
          stderr_to_stdout: true
        )
      ...
      {_, 0} =
        System.cmd("git", ["worktree", "add", worktree_path, "feature/worktree-only"],
          cd: test_dir,
          stderr_to_stdout: true
        )
  ...
  describe "worktree output parsing" do
    test "parses worktree porcelain format correctly" do
      ...
      assert Map.get(worktrees, "feature/auto-spawn-agents") ==
               "/home/martins/code/systematic-auto-spawn"

      assert Map.get(worktrees, "feature/branch-panel") ==
               "/home/martins/code/systematic-branches"
    end
  ...
  describe "understanding empty results" do
    ...
    test "panel shows empty when all branches are merged to main", %{test_dir: test_dir} do
      ...
      {_, 0} =
        System.cmd("git", ["commit", "-m", "Feature A"], cd: test_dir, stderr_to_stdout: true)
      ...
      {_, 0} =
        System.cmd("git", ["commit", "-m", "Feature B"], cd: test_dir, stderr_to_stdout: true)
      ...
      # Merge both branches
      {_, 0} =
        System.cmd("git", ["merge", "feature/a", "--no-edit"],
          cd: test_dir,
          stderr_to_stdout: true
        )

      {_, 0} =
        System.cmd("git", ["merge", "feature/b", "--no-edit"],
          cd: test_dir,
          stderr_to_stdout: true
        )
