defmodule DashboardPhoenixWeb.HomeLiveTest do
  ...
  defp switch_to_tab(view, tab) do
  ...
  describe "sub-agent task name display" do
    ...
    test "task summary is displayed with full text", %{conn: conn} do
      ...
      task_summary =
        "Improve the Sub-Agents panel: show full task names, fix clear button, and add tests"
    ...
    test "very long labels wrap properly instead of being truncated", %{conn: conn} do
      ...
      very_long_label =
        "this-is-an-extremely-long-sub-agent-label-that-describes-a-complex-task-with-many-details"
  ...
  describe "rendering" do
    ...
    test "includes CSS classes for styling", %{conn: conn} do
      ...
      assert html =~ "panel-priority" or html =~ "panel-accent" or html =~ "panel-data"
      # Note: rounded-lg removed as part of ticket #97 - plain borders
    end

    test "includes interactive elements", %{conn: conn} do
      {:ok, view, _html} = live(conn, "/")
      # Elements like clear_progress are now in overview/activity context
      # But some might be always visible
      html = render(view)
  ...
  describe "claude model selection" do
    ...
    test "claude model selection dropdown changes the claude_model assign", %{conn: conn} do
      ...
      html_sonnet =
        render_change(view, "select_claude_model", %{
    ...
    @tag :claude_tests
    test "when coding_agent_pref is :claude and claude_model is opus, work request includes opus model",
         %{conn: conn} do
      {:ok, view, _html} = live(conn, "/")
    ...
    @tag :claude_tests
    test "when coding_agent_pref is :claude and claude_model is sonnet, work request includes sonnet model",
         %{conn: conn} do
      {:ok, view, _html} = live(conn, "/")
