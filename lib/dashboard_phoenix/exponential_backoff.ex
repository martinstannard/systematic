defmodule DashboardPhoenix.ExponentialBackoff do
    ...
    case fun.() do
      ...
      {:error, reason} = error ->
        ...
        if attempt >= max_attempts do
          Logger.warning(
            "Operation failed after #{attempt} attempts. Errors: #{inspect(Enum.reverse(updated_errors))}"
          )
          ...
          Logger.debug(
            "Operation failed on attempt #{attempt}/#{max_attempts}, retrying in #{delay_ms}ms. Error: #{inspect(reason)}"
          )
  ...
  def retryable?({:exit, code, _output}) when code in [124, 125, 126, 127], do: true
  ...
  def retryable?({:exit, code, output}) do
    ...
    cond do
      ...
      String.contains?(output_lower, ["not found", "invalid", "unauthorized"]) ->
        false

      # Network/API issues
      String.contains?(output_lower, ["network", "timeout", "connection", "rate limit"]) ->
        true

      # HTTP errors
      String.contains?(output_lower, ["502", "503", "504", "429"]) ->
        true

      String.contains?(output_lower, ["temporarily unavailable", "try again"]) ->
        true

      # GitHub specific
      String.contains?(output_lower, "api rate limit exceeded") ->
        true

      String.contains?(output_lower, "abuse detection") ->
        true

      # Linear specific  
      String.contains?(output_lower, "rate limited") ->
        true

      # Exit codes suggesting temporary issues
      # Generic failures worth retrying
      code in [1, 2] ->
        true
