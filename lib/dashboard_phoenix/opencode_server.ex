defmodule DashboardPhoenix.OpenCodeServer do
  ...
  # Max attempts to check readiness
  @readiness_max_attempts 30
  # Time between readiness checks
  @readiness_interval_ms 200

  # Output buffer configuration
  # ~100KB max buffer size
  @max_output_buffer_size 100_000
  # Keep last 50KB when rotating
  @output_rotation_size 50_000

  # Circuit breaker configuration
  # Max restarts within time window
  @max_restarts 5
  # 5 minute window for restart counting
  @restart_window_ms 300_000
  # Wait 1 minute before allowing restart after circuit opens
  ...
  @session_cleanup_interval_ms 900_000
  # 1 hour idle = stale
  @session_stale_threshold_ms 3_600_000
  ...
  def init(opts) do
    ...
    state = %{
      ...
      # Circuit breaker
      # :closed (normal), :open (failing too much), :half_open (testing)
  ...
  def handle_info(:auto_start, state) do
    ...
    case do_start_server(cwd, state) do
      ...
      {:error, reason, new_state} ->
        Logger.warning(
          "[OpenCodeServer] Auto-start failed: #{inspect(reason)}, will retry on demand"
        )
    ...
    case CommandRunner.run("kill", ["-15", "#{os_pid}"], timeout: 1_000, stderr_to_stdout: true) do
      {:ok, _} ->
        ...
        if wait_for_process_exit(os_pid, @graceful_shutdown_timeout_ms) do
          ...
          Logger.warning(
            "[OpenCodeServer] Process #{os_pid} didn't exit gracefully, sending SIGKILL"
          )
  ...
  defp wait_for_readiness(port, attempts_remaining) do
    case check_server_health(port) do
      :ok ->
        Logger.info(
          "[OpenCodeServer] Server ready after #{@readiness_max_attempts - attempts_remaining + 1} attempts"
        )
  ...
  defp perform_health_check(state) do
    case check_server_health(state.port) do
      :ok ->
        %{state | health_status: :healthy, last_health_check: DateTime.utc_now()}
      ...
      {:error, reason} ->
        ...
        %{state | health_status: :unhealthy, last_health_check: DateTime.utc_now()}
    end
  end
    ...
    case state.circuit_state do
      :closed ->
        ...
        if recent_count >= @max_restarts do
          Logger.error(
            "[OpenCodeServer] Circuit breaker opened: #{recent_count} restarts in #{@restart_window_ms}ms"
          )
  ...
  defp schedule_restart(state, cwd, exit_status) do
    ...
    Logger.info(
      "[OpenCodeServer] Scheduling restart in #{delay}ms (exit status: #{exit_status}, consecutive failures: #{state.consecutive_failures})"
    )
    ...
    %{state | restart_timer: timer, next_restart_delay_ms: next_delay}
  end
  ...
  defp perform_session_cleanup(state) do
    ...
    unless state.running do
      ...
      case OpenCodeClient.list_sessions() do
        ...
        {:error, reason} ->
          Logger.warning(
            "[OpenCodeServer] Failed to list sessions for cleanup: #{inspect(reason)}"
          )
  ...
  defp delete_stale_sessions(stale_sessions) do
    Enum.reduce(stale_sessions, 0, fn session, count ->
      ...
      case OpenCodeClient.delete_session(session_id) do
        ...
        {:error, reason} ->
          Logger.warning(
            "[OpenCodeServer] Failed to delete session #{session_id}: #{inspect(reason)}"
          )
