defmodule DashboardPhoenix.MemoryUtils do
  ...
  @default_gc_interval_ms 300_000  # 5 minutes
  @memory_warning_threshold_mb 100  # Warn if process uses > 100MB
  ...
  def trigger_gc(module_name) do
    ...
    if heap_before > 125_000 do
      ...
      if freed_mb > 0.1 do  # Only log if we freed > 100KB
        Logger.debug("#{module_name}: GC freed #{Float.round(freed_mb, 2)}MB, now using #{Float.round(current_mb, 2)}MB")
      end
  ...
  def lru_evict(map, max_size, timestamp_fn) when is_map(map) and is_integer(max_size) and max_size > 0 do
  ...
  def lru_evict_list(list, max_size) when is_list(list) and is_integer(max_size) and max_size > 0 do
