defmodule DashboardPhoenix.CLICache do
  ...
  """
  @spec get_or_fetch(binary(), integer(), (-> {:ok, term()} | {:error, term()})) ::
          {:ok, term()} | {:error, term()}
  def get_or_fetch(key, ttl_ms, fetch_fn)
      when is_binary(key) and is_integer(ttl_ms) and is_function(fetch_fn, 0) do
  ...
  def stats do
    ...
    {total, valid, expired} =
      :ets.foldl(
        fn {_key, _result, expires_at}, {t, v, e} ->
