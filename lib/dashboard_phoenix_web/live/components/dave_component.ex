      ...
      <% else %>
        <%= if @main_agent_session do %>
          ...
          >
            <div class="flex items-center space-x-2">
              <span
                class={"panel-chevron " <> if(@dave_collapsed, do: "collapsed", else: "")}
                aria-hidden="true"
              >
                ▼
              </span>
              ...
              <% else %>
                <span
                  class={"px-1.5 py-0.5text-xs " <> status_badge(@main_agent_session.status)}
                  role="status"
                >
            ...
            <div class="flex items-center space-x-2">
              <% {_type, model_name, model_icon} =
                agent_type_from_model(Map.get(@main_agent_session, :model)) %>
              <span
                class="px-1.5 py-0.5bg-purple-500/20 text-purple-400 text-xs"
                title={Map.get(@main_agent_session, :model)}
                aria-label={"Using model: #{model_name}"}
              >
          ...
          <div
            id="dave-panel-content"
            class={"transition-all duration-300 ease-in-out overflow-hidden " <> if(@dave_collapsed, do: "max-h-0", else: "max-h-[400px]")}
          >
            <div class="px-3 pb-3">
