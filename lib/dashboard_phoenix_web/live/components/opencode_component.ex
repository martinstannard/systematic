  ...
  def render(assigns) do
    ...
    <div class="panel-work overflow-hidden">
      ...
      >
        <div class="flex items-center space-x-2">
          <span class={"panel-chevron " <> if(@opencode_collapsed, do: "collapsed", else: "")}>
            ▼
          </span>
          ...
          <% else %>
            <%= if @opencode_server_status.running do %>
              <span class="status-beacon text-success" title="Server Online" aria-label="Online">
                ●
              </span>
      ...
      <div
        id="opencode-panel-content"
        class={"transition-all duration-300 ease-in-out overflow-hidden " <> if(@opencode_collapsed, do: "max-h-0", else: "max-h-[400px]")}
      >
        <div class="px-4 pb-4">
          <%= if @opencode_loading do %>
    ...
    <!-- Sessions List -->
              <div
                ...
                role="region"
                aria-live="polite"
                aria-label="OpenCode sessions list"
              >
                  ...
                  <div class="px-3 py-3 panel-status hover:bg-accent/10 text-xs font-mono border border-accent/20 hover:border-accent/40 transition-all rounded">
                    ...
                    <div class="flex items-start justify-between mb-1">
                      <div class="flex items-center space-x-2 min-w-0">
                        <span
                          class={opencode_status_badge(session.status)}
                          title={"Session status: " <> String.upcase(opencode_status_text(session.status))}
                        >
                          {opencode_status_symbol(session.status)} {opencode_status_text(
                            session.status
                          )}
                        </span>
                        <span class="text-white truncate" title={session.title || session.slug}>
                          {session.slug}
                        </span>
                      </div>
                    ...
                    <%= if session.file_changes.files > 0 do %>
                      <div class="flex items-center space-x-2 text-xs">
                        ...
                        <span
                          ...
                          aria-label={"#{session.file_changes.additions} additions"}
                        >
                          +{session.file_changes.additions}
                        </span>
                        <span
                          class="text-red-400"
                          aria-label={"#{session.file_changes.deletions} deletions"}
                        >
    ...
    <!-- Directory -->
                    <%= if session.directory do %>
                      <div
                        class="text-xs text-base-content/30 truncate mt-1"
