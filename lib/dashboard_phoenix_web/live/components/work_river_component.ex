defmodule DashboardPhoenixWeb.Live.Components.WorkRiverComponent do
  ...
  def update(assigns, socket) do
    ...
    total_items = length(input_items) + length(in_progress_items) + length(review_items) + length(done_items)
  ...
  defp build_input_items(assigns) do
    ...
    chainlink_items = chainlink_issues
  ...
  defp build_in_progress_items(assigns) do
    ...
    opencode_items = opencode_sessions
  ...
  defp build_done_items(assigns) do
    ...
    # Completed sessions from last 24h
    agent_sessions
    ...
    <div class="work-river-panel overflow-hidden" id="work-river-panel">
      ...
      >
        <div class="flex items-center space-x-2">
          <span class={"panel-chevron " <> if(@work_river_collapsed, do: "collapsed", else: "")}>▼</span>
      ...
      <div id="work-river-content" class={"transition-all duration-300 ease-in-out overflow-hidden " <> if(@work_river_collapsed, do: "max-h-0", else: "max-h-[600px]")}>
        <div class="px-4 pb-4">
          <!-- River Flow Visualization -->
          <div class="work-river-flow flex gap-4 overflow-x-auto pb-2" role="region" aria-label="Work stages">
            
            <!-- INPUT LANE -->
            <div class="work-river-lane flex-shrink-0 w-64" role="region" aria-labelledby="lane-input">
              <div id="lane-input" class="flex items-center gap-2 mb-3 pb-2 border-b border-blue-500/30">
            ...
            <!-- IN PROGRESS LANE -->
            <div class="work-river-lane flex-shrink-0 w-64" role="region" aria-labelledby="lane-progress">
              <div id="lane-progress" class="flex items-center gap-2 mb-3 pb-2 border-b border-purple-500/30">
            ...
            <!-- REVIEW LANE -->
            <div class="work-river-lane flex-shrink-0 w-64" role="region" aria-labelledby="lane-review">
              <div id="lane-review" class="flex items-center gap-2 mb-3 pb-2 border-b border-orange-500/30">
                      ...
                      <div class="flex items-start justify-between gap-2 mb-1">
                        ...
                        <div class="flex items-center gap-1">
                          <%= if item.has_conflicts do %>
                            <span class="text-xs px-1 bg-red-500/20 text-red-400 rounded" title="Has merge conflicts">⚠️</span>
                          <% end %>
                          <%= if item.ci_status == "failure" do %>
                            <span class="text-xs px-1 bg-red-500/20 text-red-400 rounded" title="CI failing">❌</span>
                          <% else %>
                            <%= if item.ci_status == "success" do %>
                              <span class="text-xs px-1 bg-green-500/20 text-green-400 rounded" title="CI passing">✅</span>
                            <% end %>
                          <% end %>
                        </div>
                      </div>
            ...
            <div class="work-river-lane flex-shrink-0 w-64" role="region" aria-labelledby="lane-done">
              <div id="lane-done" class="flex items-center gap-2 mb-3 pb-2 border-b border-green-500/30">
