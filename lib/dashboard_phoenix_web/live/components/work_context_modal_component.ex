defmodule DashboardPhoenixWeb.Live.Components.WorkContextModalComponent do
  ...
  defp build_chainlink_context(context, item, assigns) do
    ...
    %{context | ticket: item.source_data, agent_session: agent_session}
  end
  ...
  defp build_agent_context(context, item, assigns) do
    ...
    %{context | ticket: ticket, agent_session: session, pr: pr}
  end
  ...
  defp build_pr_context(context, item, assigns) do
    ...
    %{context | ticket: ticket, agent_session: agent_session, pr: pr, branch: pr.branch}
  end

  defp build_completed_context(context, item, _assigns) do
    session = item.source_data

    %{context | agent_session: session}
  end
  ...
  defp build_related_items(ticket_id, assigns) do
    ...
    related_agents =
      Enum.filter(agent_sessions, fn s ->
        label = Map.get(s, :label, "")
        task = Map.get(s, :task_summary, "")
        String.contains?(label, ticket_id) or String.contains?(task, ticket_id)
      end)
        ...
        >
          ...
          <div class="flex items-center justify-between px-6 py-4 border-b border-base-300 bg-base-300/50">
            <div class="flex items-center gap-3">
              <div
                class={"w-3 h-3 rounded-full " <> item_type_color(@selected_item.type)}
                aria-hidden="true"
              >
              </div>
    ...
    <!-- TICKET/ISSUE SECTION -->
            <%= if @context.ticket || @selected_item.type in [:linear, :chainlink] do %>
              <section class="space-y-3" aria-labelledby="ticket-section">
                <h3
                  id="ticket-section"
                  class="flex items-center gap-2 text-sm font-semibold text-blue-400"
                >
                  <span class="w-2 h-2 rounded-full bg-blue-500"></span> Ticket Details
                </h3>
    ...
    <!-- AGENT SESSION SECTION -->
            <%= if @context.agent_session do %>
              <section class="space-y-3" aria-labelledby="agent-section">
                <h3
                  id="agent-section"
                  class="flex items-center gap-2 text-sm font-semibold text-purple-400"
                >
                  <span class="w-2 h-2 rounded-full bg-purple-500"></span> Active Agent
                </h3>
                <div class="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4">
                  <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-3">
                      <span class="text-2xl">
                        {agent_icon(get_agent_type(@context.agent_session))}
                      </span>
                      <div>
                        <div class="flex items-center gap-2">
                          <span class="font-medium text-base-content">
                            {Map.get(@context.agent_session, :label) ||
                              Map.get(@context.agent_session, :slug)}
                          </span>
                    ...
                    <div class="mt-3 pt-3 border-t border-purple-500/20 flex items-center gap-4 text-sm">
                      <div class="flex items-center gap-1 text-base-content/60">
                        ...
                        <span class="font-mono">
                          {format_tokens(@context.agent_session.tokens_in)}
                        </span>
                      </div>
                      <div class="flex items-center gap-1 text-base-content/60">
                        <span>↑</span>
                        <span class="font-mono">
                          {format_tokens(@context.agent_session.tokens_out)}
                        </span>
                      </div>
    ...
    <!-- PR SECTION -->
            <%= if @context.pr do %>
              <section class="space-y-3" aria-labelledby="pr-section">
                <h3
                  id="pr-section"
                  class="flex items-center gap-2 text-sm font-semibold text-orange-400"
                >
                  <span class="w-2 h-2 rounded-full bg-orange-500"></span> Pull Request
                </h3>
    ...
    <!-- RELATED ITEMS -->
            <%= if @context.related_items != [] do %>
              <section class="space-y-3" aria-labelledby="related-section">
                <h3
                  id="related-section"
                  class="flex items-center gap-2 text-sm font-semibold text-base-content/70"
                >
                  <span class="w-2 h-2 rounded-full bg-base-content/30"></span> Related Items
                </h3>
