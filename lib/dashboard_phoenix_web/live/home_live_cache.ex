defmodule DashboardPhoenixWeb.HomeLiveCache do
  ...
  @max_cache_size 100
  # 30 seconds TTL for cache entries
  @ttl_ms 30_000
  ...
  def get_graph_data(sessions, coding_agents, processes, opencode_sessions, gemini_status) do
    key =
      {:graph_data, fingerprint(sessions), fingerprint(coding_agents), fingerprint(processes),
       fingerprint(opencode_sessions), fingerprint(gemini_status)}
    ...
    case get_cached(key) do
      ...
      :miss ->
        result =
          compute_graph_data(sessions, coding_agents, processes, opencode_sessions, gemini_status)
  ...
  defp hash_item(item) do
    ...
    |> Base.encode16()
    # Use first 16 chars for performance
    |> String.slice(0, 16)
  end
  ...
  defp compute_graph_data(sessions, coding_agents, processes, opencode_sessions, gemini_status) do
    ...
    {opencode_nodes, opencode_links} =
      ...
      |> Enum.map(fn session ->
        node = %{
          ...
          type: "opencode",
          status:
            if(session.status in [Status.active(), Status.running()],
              do: Status.running(),
              else: Status.idle()
            )
        }
    ...
    {gemini_nodes, gemini_links} =
      if gemini_status.running do
        node = %{
          id: "gemini-main",
          label: "Gemini CLI",
          type: "gemini",
